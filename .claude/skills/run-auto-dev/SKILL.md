...
It runs 26 assertions and exits 0 iff all pass. The 3 live-arXiv and 1 live-cross-model checks
**SKIP** (not fail) when offline / keyless: a clean, offline, keyless box reports `pass=22
skip=4`; this box (online, key present) reports `pass=26 skip=0`. Last line looks like:

```
== RESULT: pass=26 fail=0 skip=0 ==
```
